;;; magent-profile-memory.el --- Personal Magent profile memory -*- lexical-binding: t; -*-
...
;; Personal Magent context provider and Action group.  It scans a bounded,
;; confirmed set of Emacs configuration files, writes a local, human-editable
;; Org profile, and contributes a small selected subset to relevant Magent
;; turns through `magent-context-provider-functions'.
...
(defgroup magent-profile-memory nil
  "Personal Emacs profile memory for Magent."
  :group 'magent)

(defcustom magent-memory-directory
  (expand-file-name "magent/memory/" user-emacs-directory)
  "Directory where the personal Magent profile memory is stored."
  :type 'directory
  :group 'magent-profile-memory)
...
When nil, write a deterministic skeleton and source index without sending
configuration excerpts to the provider."
  :type 'boolean
  :group 'magent-profile-memory)
...
(defcustom magent-memory-enable-auto-injection t
  "Whether relevant Emacs profile memory may be contributed to Magent."
  :type 'boolean
  :group 'magent-profile-memory)

(defcustom magent-memory-scan-custom-file nil
  "Whether memory scans may read `custom-file' contents.
When nil, record the custom file path without reading its contents."
  :type 'boolean
  :group 'magent-profile-memory)
...
(defcustom magent-memory-injection-max-chars 6000
  "Maximum characters of Emacs profile memory contributed to one prompt."
  :type 'integer
  :group 'magent-profile-memory)

(defcustom magent-memory-max-injected-sections 3
  "Maximum number of memory sections contributed to one prompt."
  :type 'integer
  :group 'magent-profile-memory)

(defconst magent-profile-memory--system-prompt
  (concat
   "You summarize an Emacs configuration into Magent's local Emacs profile "
   "memory. Use only the provided sanitized bundle. Do not infer secrets. "
   "Do not include ordinary personal knowledge-base notes. Return only Org "
   "content with this exact schema: a top-level '* {{managed-heading}}' "
   "heading and these second-level headings, in order: {{section-headings}}. "
   "Keep entries concise and operational. Include source file references "
   "where useful.")
  "System prompt template used to summarize the Emacs profile.")

(defconst magent-profile-memory--user-prompt
  (concat
   "Summarize this sanitized Emacs configuration bundle for future Magent "
   "assistance.\n\nScan roots:\n{{scan-roots}}\n\nEntry files:\n"
   "{{entry-files}}\n\nCustom file path: {{custom-file}}\n\n"
   "Sanitized bundle:\n\n{{bundle}}")
  "User prompt template used to summarize the Emacs profile.")

(defconst magent-profile-memory--injection-prompt
  (concat
   "* Magent Emacs Profile Memory\n\n"
   "This is selected, sanitized supporting context and may be incomplete or "
   "stale. Do not infer facts from missing entries. The user's current "
   "instructions and live Emacs or repository state take precedence when "
   "they conflict with this memory.\n\n{{memory}}")
  "Template used to contribute selected profile memory to a Magent turn.")

(defun magent-profile-memory--render-prompt (template replacements)
  "Render TEMPLATE by substituting named REPLACEMENTS literally."
  (let ((result template))
...
(defun magent-memory--summarizer-system-prompt ()
  ...
  (magent-profile-memory--render-prompt
   magent-profile-memory--system-prompt
...
(defun magent-memory--summarizer-user-prompt (plan bundle)
  ...
  (magent-memory--sanitize-outbound
   (magent-profile-memory--render-prompt
    magent-profile-memory--user-prompt
    `((scan-roots
       . ,(mapconcat (lambda (root) (format "- %s" root))
                     (magent-memory-scan-plan-roots plan)
                     "\n"))
...
(defun magent-profile-memory-register ()
  "Register the personal profile-memory provider and Actions."
  (add-hook 'magent-context-provider-functions
...
(provide 'magent-profile-memory)
;;; magent-profile-memory.el ends here
