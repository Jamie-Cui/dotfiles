;;; latex.el --- LaTeX authoring and SyncTeX -*- lexical-binding: t -*-
;;; Commentary:
;; AUCTeX, RefTeX, Xenops and source/PDF integration in graphical sessions.
;; PDF viewing itself belongs to pdf.el; Overleaf belongs to vc.el.
;;; Code:

(declare-function TeX-pdf-tools-sync-view "tex" ())
(declare-function TeX-revert-document-buffer "tex" (file))
(defvar TeX-source-correlate-mode)
(defvar TeX-source-correlate-start-server)
(defvar TeX-view-program-selection)
(defvar TeX-view-program-list)
(defvar TeX-PDF-mode)
(defvar pdf-sync-forward-display-action)
(defvar pdf-sync-backward-search-method)
(defvar pdf-sync-generic-forward-search-command)

(when (display-graphic-p)
  (require 'cl-lib)
  (require 'reftex)

  (use-package auctex
    :ensure t)

  ;; Set PDF-Tools as the default viewer
  (defun +latex/pdf-tools-sync-view ()
    "View the current TeX PDF with PDF Tools.

Suppress the noisy SyncTeX message shown when point is on a source
line, such as a preamble line, that has no corresponding PDF location."
    (let ((message-function (symbol-function 'message)))
      (cl-letf (((symbol-function 'message)
                 (lambda (format-string &rest args)
                   (let ((text (and (stringp format-string)
                                    (apply #'format-message format-string args))))
                     (unless (and text
                                  (string= text "epdfinfo: Destination not found"))
                       (apply message-function format-string args))))))
        (TeX-pdf-tools-sync-view))))

  ;; Either module may load first.  Register source/PDF glue only after
  ;; both libraries are available, retaining the PDF Tools platform guard.
  (unless (eq system-type 'windows-nt)
    (with-eval-after-load 'tex
      (with-eval-after-load 'pdf-tools
        ;; Refresh the viewer after TeX compilation.
        (add-hook 'TeX-after-compilation-finished-functions
                  #'TeX-revert-document-buffer)

        ;; Enable SyncTeX
        (setq TeX-source-correlate-mode t)
        (setq TeX-source-correlate-start-server t)
        (setq pdf-sync-forward-display-action nil)

        (setq TeX-view-program-selection '((output-pdf "PDF Tools"))
              TeX-view-program-list
              '(("PDF Tools" +latex/pdf-tools-sync-view)))

        ;; Enable Inverse Search in PDFs
        (setq pdf-sync-backward-search-method 'generic
              pdf-sync-generic-forward-search-command
              "emacsclient --no-wait +%l '%f'")

        (setq TeX-PDF-mode t)
        (add-hook 'LaTeX-mode-hook 'TeX-source-correlate-mode))))

  ;; utility function
  (defun +latex/isolate-sentence ()
    "Insert a TeX comment line between sentences in the selected region."
    (interactive)
    (if (use-region-p)
        (let ((start (region-beginning))
              (end (copy-marker (region-end) t)))
          (save-excursion
            (goto-char start)
            (while (re-search-forward "\\.\\(?:[ \t]+\\|\n\\)" end t)
              (unless (save-excursion
                        (skip-chars-forward " \t")
                        (looking-at-p "%"))
                (replace-match ".\n%\n" nil t))))
          (set-marker end nil)
          (message "Replacement done!"))
      (message "No region selected!")))

  (require 'init-config-xenops)

  (use-package engrave-faces
    :ensure t))

(provide 'init-lang-latex)
;;; latex.el ends here
