...
(ert-deftest agent-shell-permission-transient-uses-current-source-shell ()
  (agent-shell-permission-transient-test--isolated
    (let ((source-shell (generate-new-buffer " *permission-source*")))
      (unwind-protect
          (cl-letf (((symbol-function
                      'agent-shell-permission-transient--ensure-subscriptions)
                     #'ignore)
                    ((symbol-function
                      'agent-shell-permission-transient--schedule-popup)
                     #'ignore))
            (with-current-buffer source-shell
              (setq-local major-mode 'agent-shell-mode)
              (agent-shell-permission-transient--responder
               (agent-shell-permission-transient-test--permission 1)))
            (should
             (eq source-shell
                 (agent-shell-permission-transient--request-shell-buffer
                  (car agent-shell-permission-transient--queue)))))
        (kill-buffer source-shell)))))

(ert-deftest agent-shell-permission-transient-rejects-non-shell-context ()
  (agent-shell-permission-transient-test--isolated
    (with-temp-buffer
      (should-error
       (agent-shell-permission-transient--responder
...
(ert-deftest agent-shell-permission-transient-mode-manages-responder ()
  (agent-shell-permission-transient-test--isolated
    (let ((agent-shell-permission-responder-function nil)
          (agent-shell-permission-transient--previous-responder nil))
...
(ert-deftest agent-shell-permission-transient-acp-callback-uses-shell-context ()
  (agent-shell-permission-transient-test--isolated
    (let ((source-shell (generate-new-buffer " *permission-source*"))
          (unrelated-buffer (generate-new-buffer " *permission-unrelated*")))
      (unwind-protect
          (let* ((client
                  (acp-make-client
                   :command "true"
                   :context-buffer source-shell))
                 (state
                  (agent-shell--make-state :buffer source-shell)))
            (map-put! state :client client)
            (with-current-buffer source-shell
              (setq-local major-mode 'agent-shell-mode)
              (setq-local agent-shell--state state))
            (let ((agent-shell-permission-responder-function
                   #'agent-shell-permission-transient--responder)
                  (agent-shell-permission-transient--previous-responder nil))
              (cl-letf
                  (((symbol-function
                ...
                (with-current-buffer unrelated-buffer
                  (funcall
                   (car (map-elt client :request-handlers))
...
(ert-deftest agent-shell-permission-transient-defer-quits-menu ()
  (should
   (eq (plist-get
        (cdr (transient-get-suffix
              'agent-shell-permission-transient--menu "q"))
        :command)
       #'transient-quit-one)))
