...
1. Confirm the runtime target.
   Identify whether an Emacs server is available and whether you can inspect live state through the fixed helper functions.
...
4. Eliminate stale state.
   Refresh the smallest changed `.el` files through the package's normal load or test path, clear session state when needed, and distinguish real logic failures from stale runtime state.
...
## Hung Live Session Recovery

When a live Magent request stops responding, try a recoverable interrupt before killing Emacs:

- In a GUI frame, send `C-g` directly to the focused Emacs window; under Hyprland, focus by PID and use `hyprctl dispatch sendshortcut CTRL,G,pid:<pid>` when appropriate.
- For daemon-only reproductions, try host-side `SIGINT` first: `/bin/kill -SIGINT <pid>`. Confirm the PID from the host namespace, because sandboxed `ps`/`kill` can disagree with host PIDs.
- If `emacsclient` still reports `Server not responding`, capture a C stack with `gdb -batch -p <pid> -ex 'set pagination off' -ex 'thread apply all bt'` before terminating the daemon.
- Use `TERM` only after collecting enough evidence or when CPU/memory is still rising.
