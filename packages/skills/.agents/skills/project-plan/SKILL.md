---
name: project-plan
description: Manage plans, TODO/PROJ items, deadlines, notes, and summaries in the CalDAV-backed central project Org file below `+org-projects-dir`, resolved by `(+org-project-current-file)` or `(+org-project-file-dwim)`. Use only when the user explicitly invokes `$project-plan`; never trigger implicitly from planning language, the presence of a plan or project Org file, or a request to create, review, or update a plan. If invoked without any follow-up prompt, switch to plan mode, list the current active plans, and ask the user which one to execute or review.
---

# Project Plan

Treat the project's helper-resolved, CalDAV-backed central Org file as the source of truth for project plans, deadlines, notes, and summaries.
...
Check whether each resolver exists before calling it. If neither exists, stop
and report that the Emacs project-note integration is unavailable. Do not
create or guess a fallback file.
...
- Keep every project TODO under the helper-resolved file in `+org-projects-dir`
- Never fall back to `project.org` in the active repository or to the retired `~/opt/org-root/projects/` directory
...
1. Resolve the central file with `(+org-project-current-file)` or `(+org-project-file-dwim)`.
2. Verify that the result is below `+org-projects-dir`; stop without writing when the resolver or directory configuration is unavailable or stale.
...
- Refuse to write when a resolver points outside the configured CalDAV-backed project directory
