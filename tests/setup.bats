...
@test "Ice apply installs the Homebrew cask" {
	prepare_mock_environment
	run env SETUP_TEST_PLATFORM=macos \
		"$setup_script" apply --profile minimal --with ice \
		--yes --repo-dir "$mock_repo"
	[ "$status" -eq 0 ]
	run grep -F 'brew install --cask jordanbaird-ice' "$SETUP_TEST_LOG"
	[ "$status" -eq 0 ]
}
...
@test "source-only components clone with their declared history depth" {
	...
	[ "$status" -eq 0 ]
	run grep -F "git clone ssh://git@git.jamie-cui.com:2222/jamie/org-root.git $HOME/opt/org-root" "$SETUP_TEST_LOG"
	[ "$status" -eq 0 ]
