...
platform profile.  With a terminal attached, the one-line command opens a
numbered component menu:
...
The setup script's =emacs= component installs the dependencies, safely clones
or fast-forwards the =emacs-31= source, and runs =autogen.sh= and =configure=.
Compilation and installation remain manual unless =--build-emacs= is supplied;
that explicit mode uses at most four jobs by default and requires 10 GiB of
available space.
...
The =ctags= component shallow-clones the source into =~/opt/ctags=.  It does
not configure, build, or install Universal Ctags.
...
The =org-root= component clones the private repository over SSH into
=~/opt/org-root=, which is the default path used by the managed Emacs
configuration.  GitHub SSH access must already be configured.
...
The =tdlib= component shallow-clones the source into =~/opt/tdlib=.  It does
not run CMake, build, or install TDLib.
