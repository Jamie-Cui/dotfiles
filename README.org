...
./autogen.sh
./configure --with-ns --without-native-compilation --with-tree-sitter --with-sqlite3 --with-rsvg --without-pop --without-mailutils
...
=ssh://git@git.jamie-cui.com:2222/jamie/org-root.git= with its full history over
SSH into =~/opt/org-root=, which is the default path used by the managed Emacs
configuration.  SSH access to =git.jamie-cui.com= on port =2222= must already be
configured.
